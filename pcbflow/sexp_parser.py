...
class Sexp(object):
    ...
    def _export(self, out, prefix="", indent="  "):
        ...
        try:
            ...
            if p is not None:
                p(out, prefix, indent)
            elif isinstance(self._value, string_types) or not hasattr(
            ...
            logger.error(
                (len(prefix) / len(indent), self.__class__.__name__, self._key, str(e))
            )
    ...
    def _getError(self):
        ...
        ret = []
        for v in self._value if isinstance(self._value, list) else self._value.values():
            try:
                ret += v._getError()
            except:
                pass
            ...
            try:
                ...
                if parse is not None:
                    if log:
                        logger.info(
                            "{}._pos{}_parse {}".format(
                                self.__class__.__name__, i, entry
                            )
                        )
                    ...
                    if parse is not None:
                        if log:
                            logger.info(
                                "{}._parse1_{} {}".format(
                                    self.__class__.__name__, subkey, entry
                                )
                            )
                        action = 1  # force one instance, and raise error if not
                    else:
                        action = (
                            2  # always use SexpList even if there is only one instance
                        )
                ...
                if parse is None:
                    ...
                    if log:
                        logger.info(
                            "{}._parse {} {}".format(self.__class__.__name__, i, entry)
                        )
...
class SexpBool(Sexp):
    ...
    def __init__(self, data):
        ...
        if value not in SexpBool._yes_values and value not in SexpBool._no_values:
